 ...
 * 
 * String projectNumberOfUnivasAppGcm = "49731260058"; try {
 * 
 * // 1. URL para envio das mensagens URL url = new
 * URL("https://android.googleapis.com/gcm/send");
 * 
 * // 2. Abre a conexão com o serviços a google HttpURLConnection conn =
 * (HttpURLConnection) url.openConnection();
 * 
 * // 3. Especifica o metodo de envio conn.setRequestMethod("POST");
 * 
 ...
 * 
 * // 5.2 Get connection output stream DataOutputStream wr = new
 * DataOutputStream(conn.getOutputStream());
 * 
 * // 5.3 Copy Content "JSON" into mapper.writeValue(wr, content);
 * 
 * // 5.4 Send the request wr.flush();
 * 
 * // 5.5 close wr.close();
 * 
 * // 6. Get the response int responseCode = conn.getResponseCode();
